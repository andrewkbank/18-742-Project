...
main(int argc, char **argv)
{
    ...
    for (int iter = 0; iter < 2; ++iter) {
        ...
        for (int j = 0; j < col_length; ++j) {
            shift_row_left_bytes(output[j], vals1[j], per_col_bytes, shift_amount);
        }
    }
    ...
    int mismatches = 0;
    for (int j = 0; j < col_length; ++j) {
        shift_row_left_bytes(expected[j], vals1[j], per_col_bytes, shift_amount);
    ...
    uint64_t checksum = checksum_vector_array(output, col_length,
                                              per_col_bytes);
    print_shift_summary("shift_left_arbitrary-baseline", pattern, shift_amount,
                        checksum, mismatches);
